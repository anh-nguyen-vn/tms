...
//		$("#datetimepicker").datetimepicker().mask("99/99/9999 99:99:99");
        $("#userid").mask("UID99999999");
  ...
  scrollmenu();
	$("#shadowhead").show();
		...
		if($(window).width()<=768){
			...
			$("#expandbtn").show();
			if ($.cookie('menu_expand')) {
					$('body').addClass('nobg');
					$('#content').css({ marginLeft: "40px" });	
					$('#main_menu').hide();	
					$('#expandbtn').css({ left: "0px" });
					$("#expandbtn .off").hide();
			}else{
				$('body').removeClass('nobg').addClass('dashborad');
				$('#content').css({ marginLeft: "232px" });	
...
////////////////////// add Script  edit:nobuhiro.miyagi ////////////////////

// SideMenuExpand
function MenuExpand(){
		$("#expandbtn .off").click(function(){
			$.cookie('menu_expand', 'off', { expires: 6});
			$('#content').css({ marginLeft: "40px" });
			$('#main_menu').hide();	
			$('#expandbtn').css({ left: "0px" });
			$("#expandbtn .on").show();
		...
		$("#expandbtn .on").click(function(){
			$.cookie('menu_expand', null); 
			$('body').removeClass('nobg').addClass('dashborad');
			$('#content').css({ marginLeft: "232px" });	
			$('#main_menu').removeClass('iconmenu ').addClass('main_menu');
			$('#main_menu li a').find('b').show();	
			$('#main_menu').show();	
			$('#expandbtn').css({ left: "200px" });
			$("#expandbtn .on").hide();
			$("#expandbtn .off").show();
		});
}
...
$(document).ready(function(){
		...
		$("#hideboxcotl").click(function(){
			$('#hidebox').toggle('blind', '', 300 );
			$('#hideboxcotl span').toggleClass("on");
		});
});
